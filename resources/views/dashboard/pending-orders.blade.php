...
<x-admin-layout :dash="$nope" :prod="$nope" :ord="$active">
    <main>
        <h1 class="m-3">Pending Orders</h1>
        <div class="p-5 container row">
            @foreach ($orders as $order)
                @if ($order->status != "completed")
                    <div class="border border-primary border-2 p-2 m-1 col-3">
                        @php
                        ...
                        <a href="{{ route('orderDetails', $order->id) }}" class="btn btn-primary float-end">View Details</a>
                    </div>
                @endif
            @endforeach
        </div>
